...
// 2. Add account entries for From and To Accounts
// 3. Update the accounts' balance
// within a single database transtaction
func (store *SQLStore) TransferTx(ctx context.Context, arg TransferTxParams) (TransferTxResult, error) {
	...
	err := store.execTx(context.Background(), func(q *Queries) error {
		...
		// We want to update account ID in order to avoid deadlocks, see deadlock test in store_test
